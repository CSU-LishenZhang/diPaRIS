...
class GlobalExpectationPooling1D(keras.layers.Layer):
    ...
    def call(self, x, **kwargs):
        if self.mode == 0:
            ...
            # x = x - max(x)
            diff_1 = tf.subtract(now, tf.reduce_max(now, axis=-1, keepdims=True))# keep_dims改成了keepdims
            # x = mx
    ...
    def build(self, input_shape):
        if self.m_trainable:
            self.m = self.add_weight(name='m',
                                     shape=(1, 1),
                                     initializer=keras.initializers.Constant(value=self.m_value),
                                     trainable=True)
        else:
            self.m = self.add_weight(name='m',
                                     shape=(1, 1),
